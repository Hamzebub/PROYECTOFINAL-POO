...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            SELECT 0[Respuesta], 'El n�mero de documento ya se encuentra registrado.'[Mensaje],0 [Id]
            RETURN
        END
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            RETURN
        END
        -- Verificar que el n�mero de documento no pertenezca a otro cliente
        IF EXISTS (
            SELECT 1 FROM Cliente WHERE NumeroDocumento = @NumeroDocumento AND Cliente_Id <> @Cliente_Id
        )
        BEGIN
            SELECT 0[Respuesta],'El n�mero de documento ya est� registrado.' [Mensaje],0 [Id]
            RETURN
        END
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            SELECT 0[Respuesta],'El número de documento ya está registrado en otro Usuario.' [Mensaje],0 [Id]
            RETURN
        END
